...
if page == "Sales Overview":
    ...
    if filter_type == "Category":
        ...
        data = df[df['Category'] == target].groupby('Order Date')['Sales'].sum().resample('M').sum().reset_index()
    elif filter_type == "Region":
        target = st.selectbox("Select Region", df['Region'].unique())
        data = df[df['Region'] == target].groupby('Order Date')['Sales'].sum().resample('M').sum().reset_index()
    else:
        target = "Overall Sales"
        data = df.groupby('Order Date')['Sales'].sum().resample('M').sum().reset_index()
